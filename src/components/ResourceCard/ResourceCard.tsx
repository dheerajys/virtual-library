...
/**
 * ResourceCard component for displaying library resources
 * Supports both normal and manage modes via isManageMode prop
 */
...
/**
 * Build the final URL for a resource based on realFilename and url properties
 */
const buildResourceUrl = (resource: IResource): string => {
  const baseUrl = 'https://www.connexus.com';
  
  // If realFilename is not null/empty, use launch URL
  if (resource.realFilename && resource.realFilename.trim() !== '') {
    return `${baseUrl}/library/launch.aspx?id=${resource.id}`;
  }
  
  // If url starts with https, use it directly
  if (resource.url.toLowerCase().startsWith('https')) {
    return resource.url;
  }
  
  // Otherwise, append the url value to base URL
...
const ResourceCard: React.FC<ResourceCardProps> = ({
  ...
  const handleCardClick = () => {
    if (onClick) {
      ...
      // Default behavior: open the properly formatted URL
      const finalUrl = buildResourceUrl(resource);
